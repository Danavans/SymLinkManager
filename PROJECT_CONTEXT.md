...
## Current UI — 1.1.0
Graphite dark surfaces and indigo accents; green = healthy, rose = broken and amber = unreadable. Sidebar with Scan & export and Import & recreate plus a discreet bottom-aligned version label. The compact scan workspace prioritizes the inventory: health filters, comma-separated OR search and `-term` exclusions, sortable inventory, 100-row pagination and filtered export across all pages. Import is a four-stage flow with a debounced background preview, detected roots, optional editable mapping rules and replacement confirmation; the last valid preview remains visible while an update is pending. Native HTML dialogs contain focus. Status is visible in the bottom bar. Narrow windows reflow navigation and panels. The native window remains hidden while its capped 75% first-launch geometry or saved physical client size, position and maximized state is applied, then shows once against a graphite native background; an off-screen saved position falls back to centering.
